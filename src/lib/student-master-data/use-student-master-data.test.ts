...
function storedRecord(seasonId: string, className: string) {
  return {
    userId: STUDENT,
    seasonId,
    eventId: null,
...
describe("useStudentMasterData", () => {
  ...
  it("reports no season while none is active", async () => {
  ...
  it("asks for the records belonging to this student", () => {
    ...
    expect(collection).toHaveBeenCalledWith(expect.anything(), "studentMasterData");
  ...
  it("returns the record of the active season", async () => {
    const { result } = renderHook(() => useStudentMasterData(STUDENT));
    signIn();
  ...
  it("ignores what the student registered for in another season", async () => {
    const { result } = renderHook(() => useStudentMasterData(STUDENT));
    signIn();
  ...
  it("hides a record that no longer matches its schema instead of showing it half-read", async () => {
    ...
    emit([doc(`s1__${STUDENT}`, { seasonId: "s1", class: 42 })]);
