...
- An assignment dialog exists, scoped to all students registered for the active season (see US-4). Students who answered "no" to "Are you attending the sports week?" (see US-11) are counted only in the per-class card's total and attendance-percentage figures; they are excluded from the selection lists and from every other statistic (male, female, skill level per program), since only attending students can be assigned to an event.
- The figures are shown as cards stacked one above the other, in the order of the list they belong to. Each card carries a collapsible triangle in front of its title, pointing right while the card is folded and down while it is open, and every card folds on its own. The title is followed by a colon and the card's headline figure — the class's registrations, the event's assigned students — which is what moves when a student is assigned, since the figures below it only count the answers a student has actually given.
...
- Below the per-class cards, a card per event of the season shows the same attending-only figures — male, female, and the skill-level matrix — with no total and no attendance percentage, since every student counted there is by definition attending. It only counts students assigned to that event.
...
- Clicking anywhere on an event card selects that event, and the whole card is then shown in the accent colour; exactly one event is selected at a time, the first one to begin with, so there is always a week being worked on. Folding a card is the exception: it changes only what that card shows, not which event is selected.
- Below the cards, two lists show students for the selected event, one above the other: the upper list shows students not yet assigned to any event, the lower one the students assigned to the selected event.
- The teacher can select one or more students in either list (multi-select is supported). A selected student is shown by their whole row being in the accent colour; there is no checkbox. Selecting happens on the press rather than on the release, so a press that turns into a drag is already carrying what it picked; releasing a press that landed on an already selected student deselects them.
- Each list's first entry is "Alle", which is not a student but selects every student the list's filter currently leaves, and deselects them again once they are all selected. It carries a drag handle like any other row, and dragging it moves every student the filter leaves. It is not shown while the filter leaves nobody, since it would then have nothing to select.
- While an assignment is being saved, the whole dialog is out of reach and the application's shared spinner says so, because the figures and both lists are drawn from the records that write is changing.
...
- The selection is moved to the other list by dragging and dropping, which is the only way a student changes list. A student's whole row is what a pointer drags, and the grip handle at its left is what a keyboard drags. Dragging uses the application's one drag-and-drop mechanism (see Drag and Drop), so it works with mouse, touch and pen alike, and the keyboard gesture described there moves a student between the lists without a pointer at all. Dragging a student who is part of the selection takes the whole selection along; dragging one who is not moves them alone.
- To move a student from one event to a different event, the teacher first moves the student from the lower list back to the upper one (unassigning them), then selects the other event and moves the student from the upper list into its lower one; no direct move-between-events action exists.
- Both lists can be filtered by class (see US-6), by gender, by program (see US-5), by skill level (see US-7), and by a free-text filter that searches the first name and last name.
- Beside each list, starting level with the list's title, a free-text filter field for the name is shown, with a clear button (using a suitable icon) to reset it; below it, a single wrapping row of tags contains all the class, gender, program, and skill level filter options together, in that order.
