...
function seedEventSeries(id: string, isArchived: boolean) {
...
function seedRecord(id: string, eventSeriesId: string, fields: Record<string, unknown>) {
  firestore.seed("registrations", id, { userId: `u-${id}`, eventSeriesId, ...fields });
}

describe("assertNotInUse — categories matched through a master data field", () => {
  it("blocks an item selected by a record of a non-archived event series", async () => {
    seedEventSeries("open", false);
  ...
  it("allows an item used only by records of archived event series", async () => {
    seedEventSeries("done", true);
...
describe("assertEquipmentNotInUse", () => {
  it("blocks an entry a student of a non-archived event series rented", async () => {
    seedEventSeries("open", false);
  ...
  it("allows an entry only rented by students of archived event series", async () => {
    seedEventSeries("done", true);
...
describe("usageReport", () => {
  ...
  it("leaves a program whose equipment is only rented in archived event series alone", async () => {
    seedEventSeries("done", true);
    seedRecord("r1", "done", { class: "3AHIT", program: "Ski", rentedEquipment: ["Helm"] });
