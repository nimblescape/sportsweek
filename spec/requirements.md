...
- The report's tag row carries further categories the assignment dialog has no use for: the event a student is assigned to (see US-12), equipment rental, season pass (see US-10), bus pickup point (see US-8), food/diet (see US-9), and health. Equipment rental offers both answers as tags, "Leihausrüstung" and "Keine Leihausrüstung"; a student whose program asks for no equipment was never put the question and is left out of both. Health offers a single tag, "Krankheit oder Medikamente", holding the students who wrote something in the health notes field or answered that they carry medication (see US-11) — a teacher looks for the students to be aware of, and everybody else is simply the rest. Those two tags name what they gather rather than the category they sit under, since the row carries no headings. The three answer-list categories take their tags from the teacher's own lists, and the food category ends with "Sonstiges", which students are offered without it being a row a teacher maintains (see US-9).
