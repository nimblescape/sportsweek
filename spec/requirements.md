...
One drag-and-drop mechanism is used everywhere in the application: for ordering lists (US-4 through US-10) and for moving students between the cards of the assignment page (US-12). It is specified once here rather than per story, so the gesture a user learns in one place works in every other.
...
- The user record's ID is the Firebase Auth uid. It is opaque, it is not a fact about the person, and nothing can move it — so a directory rename updates the record rather than creating a second one, and no address ever appears in a document path (see US-31).
- The address the school issued is stored as a field. The account type is derived from its domain once, at creation (US-3); nothing else is decided by it.
- The authorization code flow is not run by this application. Sign-in hands the user to Firebase's own auth handler, which exchanges the code with Entra ID using a client secret held in the Firebase console. The application never sees an authorization code and holds no client secret, so how that exchange is hardened — PKCE among it — is Firebase's to implement. What is ours is configuring the directory so that the flow it implements is the one that actually runs.
- The Entra ID app registration is therefore a **Web** platform, not a single-page application. A SPA registration is a public client: it cannot hold the client secret the handler authenticates with, so the exchange would rest on the redirect URI alone. Which platform is registered is what decides whether a secret protects the code exchange at all.
- Its redirect URIs are exactly the Firebase auth handlers of the environments it serves — `https://<auth-domain>/__/auth/handler` — and nothing else. A redirect URI names a place an authorization code may be delivered to, so an extra one is one more place it may be delivered to. Each Firebase project has an auth domain of its own, so an environment is a URI added deliberately rather than a wildcard left open.
- The Firebase project's authorized domains follow the same principle. A new project admits only `*.firebaseapp.com` and `*.web.app`; the App Hosting domain, the custom domain and `localhost` are added by hand, and nothing else is.
- The client secret expires. Renewing it is tracked outside the application, because a lapsed secret stops every sign-in and nothing in the application can say why.
- The provider names the school's tenant, so the flow admits that one directory rather than every Microsoft account and every personal one (US-32).
...
- An archived season is read-only in what it holds: it can be unarchived or deleted, but not renamed. Its row therefore offers no edit control, and the server refuses a rename outright so a bypassed client cannot make one either. Its place in the list is not part of that — ordering says how a teacher wants to look at the list, not what a season is, so an archived season is dragged like any other. Its events (see below) can be read but not added to, edited, removed or reordered.
...
Removing an item and renaming one both leave the records that already hold it exactly as they are, and those records can only belong to an archived season — the rule above is what allowed the edit in the first place. Both dialogs say so before the teacher confirms: a removed item stays in that data, and a renamed one keeps the name it was given there.
...
- The list is pre-populated, in this order, with four skill levels shown as "Einsteiger:in" (starting from scratch), "Anfänger:in" (beginner), "Fortgeschritten" (advanced), and "Profi" (expert).
...
- The form is grouped into cards: the registration itself (name, class, and whether they are coming), personal details, the emergency contact, everything about the event series, and health. The event series card is titled with the event series' name rather than with a word like "programme" or "sports week", which is where the student reads which series they are filling in for; the name is not repeated as a field of its own.
- That card holds the program and the equipment it needs, the body measurements the rental asks for, and then the answers taken from the teacher's lists, in the order the master data menu shows them (see US-14). It is one card rather than three because the answers in it are about one thing, and a student reads them together.
...
- While an assignment is being saved, the whole page is out of reach and the application's shared spinner says so, because every card is drawn from the records that write is changing.
...
- The page is opened by either `viewReports` or `editReports` (see US-2). The two exclude each other, so a teacher holds one of them: whoever may only look at the report, and whoever may also save what they have set up.
...
- The report is a master-detail list rather than a table: each student is one master line showing that student's number in the list, followed by the first name and last name, followed by that student's detail lines. The number is what lets a teacher check a printed sheet off against the students actually present, and reconcile the count at the end of the day, rather than a fact stored on any record.
...
- The report has two independent tag lists: a filter tag list that determines which students are shown, and a fields tag list that determines which detail lines are shown below each master line. Above them is a third tag list, of the saved reports the two can be restored from. Each of the three sits in a card of its own, in that order from the top: saved reports, filter, fields. The filter card and the fields card are headed with what they decide, since two rows of tags under one another otherwise look like one long row split in two.
- The filter tag list works the same way as in the assignment page (see US-12): a free-text filter for the name with a clear button, and a wrapping tag row (with a first "all" tag) for class, gender, program, and skill level, each following the same within-category-OR / across-category-AND combination rules. In addition, since this report (unlike the assignment page) also lists students who answered "no", the tag row has an extra attendance category with the tags "attending" and "not attending" (see US-11), following the same selection rules as the other categories.
- The report's tag row carries further categories the assignment page has no use for: the event a student is assigned to (see US-12), equipment rental, season pass (see US-10), bus pickup point (see US-8), food/diet (see US-9), and health. Equipment rental offers both answers as tags, "Leihausrüstung" and "Keine Leihausrüstung"; a student whose program asks for no equipment was never put the question and is left out of both. Health offers a single tag, "Krankheit oder Medikamente", holding the students who wrote something in the health notes field or answered that they carry medication (see US-11) — a teacher looks for the students to be aware of, and everybody else is simply the rest. Those two tags name what they gather rather than the category they sit under, since the row carries no headings. The three answer-list categories take their tags from the teacher's own lists, and the food category ends with "Sonstiges", which students are offered without it being a row a teacher maintains (see US-9).
- Where a tag reads as its own category would — "Teilnahme" under the attendance category, "Leihausrüstung" under equipment rental — it is that word alone that names it, rather than the category repeated in front of it.
- The categories appear in the order the fields tag list shows the same answers in, so a teacher reading the two rows one under the other finds an answer in the same place in both.
- The fields tag list lets the teacher activate the data fields shown as detail lines, beyond the number, first name and last name already on the master line, in this order: attending status (yes/no, see US-11), the event a student is assigned to (see US-12), class (see US-6), gender (see US-11), date of birth (see US-11), contact data (e-mail address, phone number and emergency contact — name, relationship, and phone number — see US-1, US-11), program (see US-5), needed rental equipment (see US-11), body measurements (weight, height, shoe size, see US-11), skill level (see US-7), season pass (see US-10), bus pickup point (see US-8), food/diet option (including its free text if "other" is selected, see US-9), health/medication (health notes and whether medication is carried, see US-11), and whether the registration is still missing answers.
- The rental equipment and the body measurements stand next to the program, because the shoe size and the height are asked for in order to rent. The three lists a teacher maintains for these answers — season passes, bus pickup points and food/diet options — follow in the order the master data menu shows them (see US-14), so the report reads them back in the order they were filled in.
...
- The fields tag row's very first tag is "Keine", which deactivates every field at once and is highlighted while none is active. It is to the detail lines what the filter row's "all" tag is to the students: the report reduced to its master lines.
...
- The teacher decides the order of the tags by dragging them (see Drag and Drop and Ordering), and that order is what every teacher sees. The whole tag is what a pointer drags and the grip handle at its left is what a keyboard drags, as in the assignment cards (see US-12): a drag only begins once the pointer has moved, so a press that stays still opens the report and a press that travels moves the tag without opening anything.
...
- Pressing a tag marks it, and it stays marked while the teacher goes on changing the two tag lists — that mark is what says which saved report is being worked on, and it moves only when another tag is pressed. What pressing the marked tag again does depends on whether the report has been changed since it was opened: where it has, the saved report is put back on screen, both tag lists at once, and the tag stays marked; where it has not, the tag is released and both tag lists are left exactly as they are, since letting go of an untouched report is not asking for one.
- Putting it back is the only way to undo an edit to an opened report — the alternative is a teacher remembering every tag they pressed since. It is the name that does it, not the controls beside it and not the grip that drags the tag: those keep their own jobs, so nothing is undone by reaching for the wrong part of the tag.
...
- Taking the new name stores the report as it currently stands along with it, so a report that had been changed is no longer marked as changed afterwards. A teacher renaming the tag they have been working in means the report they are looking at, not its name on its own — leaving the two apart is how a rename could otherwise be confirmed and the report still read as unsaved.
...
- The leftmost columns are the student's number in the report and the two fields every student has and the report always shows — first name and last name — each in a column of its own. They are followed by one column per activated field, in the order the fields tag list lists them; the e-mail address is one of them, under contact data, rather than a column of its own.
...
- A student who signs in through Entra ID is admitted and continues as themselves rather than being offered a choice: following an invitation link (US-23) is a student's own path through the real sign-in, and it has to be walkable here too.
