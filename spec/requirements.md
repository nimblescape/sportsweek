...
- Each list's first entry is "Alle", which is not a student but selects every student the list's filter currently leaves, and deselects them again once they are all selected.
...
- Beside each list, a free-text filter field for the name is shown, with a clear button (using a suitable icon) to reset it; below it, a single wrapping row of tags contains all the class, gender, program, and skill level filter options together, in that order.
