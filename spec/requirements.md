...
- Beside the tags, a Save button lets the teacher save the report as it currently stands under a name. Pressing it turns the button itself into a name field with a confirm and a cancel icon, in the same place in the row; the button comes back as soon as the name is taken or the naming is abandoned, and the tag for the new report joins the row. The tag it creates is the marked one from that moment, and whichever tag was marked before is released with it: the report on screen and the tag that says so stay the same thing.
...
- Renaming a saved report edits its name in place: the tag itself becomes a name field with a confirm and a cancel icon, holding the name it had, and the tag returns as soon as the new name is taken or the rename is abandoned. Every edit is made where the thing being edited stands, so the row neither grows a line nor moves the tags around while a teacher works in it.
...
- Under the title stands what the copy holds, on every page rather than once at the foot of one. Where what is on screen is a saved report (see US-13), its name is the first of those lines; the filter that produced the export is the next — the name being searched for, then each restricted category with the tags chosen in it, so a reader can tell which slice of the season is in front of them without the screen it was taken from. A report matching no saved one carries the filter line alone, and a report of the whole season that no saved report names has no subtitle at all.
