...
- Beside the tags, a Save button lets the teacher save the report as it currently stands under a name, entered inline without leaving the report page. Saving one releases whichever tag was marked, because what was open is not what was just saved.
...
- The "Overview" sheet carries the HTL logo, and below it the report title "Sportsweek Report", the date and time at which the export was made, and — where what is on screen is a saved report (see US-13) — that saved report's name. A report matching no saved one is left unnamed rather than described tag by tag, as in the PDF.
