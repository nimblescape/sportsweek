...
- The fields tag list lets the teacher activate the data fields shown as detail lines, beyond the first name, last name and email address already on the master line, in this order: attending status (yes/no, see US-11), the event a student is assigned to (see US-12), class (see US-6), gender (see US-11), date of birth (see US-11), contact data (phone number and emergency contact — name, relationship, and phone number — see US-11), program (see US-5), needed rental equipment (see US-11), body measurements (weight, height, shoe size, see US-11), skill level (see US-7), season pass (see US-10), bus pickup point (see US-8), food/diet option (including its free text if "other" is selected, see US-9), health/medication (health notes and whether medication is carried, see US-11), and whether the registration is still missing answers.
