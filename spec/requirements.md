...
- Clicking anywhere on an event card selects that event, and the whole card is then shown in the accent colour; at most one event is selected at a time. Folding a card is the exception: it changes only what that card shows, not which event is selected.
...
- The teacher can select one or more students in either list (multi-select is supported). A selected student is shown by their whole row being in the accent colour; there is no checkbox.
- Each list's first entry is "Alle", which is not a student but selects every student the list's filter currently leaves, and deselects them again once they are all selected. It is not shown while the filter leaves nobody, since it would then have nothing to select.
